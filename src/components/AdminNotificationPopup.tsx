                  ...
                  {stats.alerts.recent.slice(0, 3).map((alert) => (
                    ...
                    >
                      ...
                      <div className="text-gray-600 mt-1">
                        Tipo: {alert.alert_type} | Severidade: {getSeverityText(alert.severity)}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {stats.pendingMusic !== undefined && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4 text-center">
                    <Music className="h-5 w-5 sm:h-6 sm:w-6 text-blue-600 mx-auto mb-2" />
                    <div className="text-xl sm:text-2xl font-bold text-blue-900">{stats.pendingMusic}</div>
                    <div className="text-xs sm:text-sm text-blue-700">Músicas Pendentes</div>
                  </div>
                )}

                {stats.recentBans !== undefined && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-3 sm:p-4 text-center">
                    <Users className="h-5 w-5 sm:h-6 sm:w-6 text-red-600 mx-auto mb-2" />
                    <div className="text-xl sm:text-2xl font-bold text-red-900">{stats.recentBans}</div>
                    <div className="text-xs sm:text-sm text-red-700">Banimentos (7d)</div>
                  </div>
                )}

                {stats.criticalLogs24h !== undefined && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 sm:p-4 text-center">
                    <FileText className="h-5 w-5 sm:h-6 sm:w-6 text-yellow-600 mx-auto mb-2" />
                    <div className="text-xl sm:text-2xl font-bold text-yellow-900">{stats.criticalLogs24h}</div>
                    <div className="text-xs sm:text-sm text-yellow-700">Logs Críticos (24h)</div>
                  </div>
                )}

                {stats.activeUsersToday !== undefined && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4 text-center">
                    <Eye className="h-5 w-5 sm:h-6 sm:w-6 text-green-600 mx-auto mb-2" />
                    <div className="text-xl sm:text-2xl font-bold text-green-900">{stats.activeUsersToday}</div>
                    <div className="text-xs sm:text-sm text-green-700">Usuários Ativos</div>
                  </div>
                )}
              </div>

              {/* Estatísticas de Review para Admin */}
                ...
                <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 sm:p-4">
                  <div className="flex items-center justify-center space-x-2">
                    ...
                    <div className="text-center">
                      <div className="text-xl sm:text-2xl font-bold text-purple-900">{stats.reviewsThisMonth}</div>
                      <div className="text-xs sm:text-sm text-purple-700">Reviews de Música Este Mês</div>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              {stats.pendingReviews !== undefined && (
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 sm:p-4 text-center">
                  <FileText className="h-5 w-5 sm:h-6 sm:w-6 text-orange-600 mx-auto mb-2" />
                  <div className="text-xl sm:text-2xl font-bold text-orange-900">{stats.pendingReviews}</div>
                  <div className="text-xs sm:text-sm text-orange-700">Revisões Pendentes</div>
                </div>
              )}

              {stats.reviewsThisMonth !== undefined && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4 text-center">
                  <CheckCircle className="h-5 w-5 sm:h-6 sm:w-6 text-blue-600 mx-auto mb-2" />
                  <div className="text-xl sm:text-2xl font-bold text-blue-900">{stats.reviewsThisMonth}</div>
                  <div className="text-xs sm:text-sm text-blue-700">Revisões Este Mês</div>
                </div>
              )}
            </div>
          )}
