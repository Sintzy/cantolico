...
export default function SongPageClient({ initialSong, songId }: SongPageClientProps) {
  ...
  const id = songId;
  const hasInitialSong = React.useRef(Boolean(initialSong));
  ...
  const [song, setSong] = React.useState<SongData | null>(initialSong);
  ...
  const [loading, setLoading] = React.useState(!initialSong);
  ...
  React.useEffect(() => {
      const fetchSong = async () => {
        try {
          if (!hasInitialSong.current) setLoading(true);
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-8 py-6 sm:py-8 md:py-14 pb-20 sm:pb-8 flex flex-col gap-6 sm:gap-8 md:gap-10">
        ...
        <main className="flex-1 min-w-0 space-y-10">
          ...
          {(pdfFilesForDisplay.length > 0 || validAudioFiles.length > 0) && (
            <section className="bg-white rounded-2xl p-6 md:p-10 border border-stone-200">
              {(() => {
                const pdfFiles = pdfFilesForDisplay;
                const audioFiles = validAudioFiles;
