...
export default function MusicPageClient({ initialSong }: MusicPageClientProps) {
  const [song] = React.useState<SongData>(initialSong);
  ...
  return (
    <div className="relative w-full min-h-screen bg-white">
      ...
      <div className="relative h-64 md:h-80 w-full flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center w-full">
          ...
          <div className="flex gap-3 justify-center mt-2">
            <StarButton songId={song.id as string} className="bg-white/20 hover:bg-white/40 text-white border-white/30 shadow" />
            <AddToPlaylistButton songId={song.id as string} className="bg-white/20 hover:bg-white/40 text-white border-white/30 shadow" />
            {pdfUrl && (
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 md:hidden bg-white/90 backdrop-blur border-t border-blue-100 flex justify-around py-2 shadow-lg">
        <StarButton songId={song.id as string} className="text-blue-700" />
        <AddToPlaylistButton songId={song.id as string} className="text-blue-700" />
        {pdfUrl && (
